...
# Profesjonalna optymalizacja pod 50+ jednoczesnych użytkowników
if "sqlite" in db_url:
    # check_same_thread=False pozwala wielu wątkom FastAPI korzystać z bazy.
    # timeout=15 sprawia, że jeśli baza jest ułamek sekundy zajęta, zapytanie poczeka zamiast rzucać błędem.
    engine = create_engine(db_url, connect_args={"check_same_thread": False, "timeout": 15})
    
    # Odpalenie trybu WAL (Write-Ahead Logging) dla SQLite - gigantyczny skok wydajności przy zapisie.
    ...
    # Konfiguracja dla PostgreSQL na serwerze (np. Render)
...
class User(Base):
    ...
    role = Column(String, default="EMPLOYEE") # Role: EMPLOYEE, TEAM_LEADER, MANAGER, ADMIN, SUPER_ADMIN
...
class EvaluationLog(Base):
    ...
    rating = Column(Integer, default=3) # Nowe: Śledzenie przyznanej oceny
...
# Bezpieczne Migracje (dodawanie kolumn bez kasowania danych)
with engine.connect() as conn:
    try: 
        conn.execute(text("ALTER TABLE evaluation_logs ADD COLUMN rating INTEGER DEFAULT 3"))
        conn.commit()
    except: 
        conn.rollback()
        
    try: 
        conn.execute(text("ALTER TABLE users ADD COLUMN hire_date TIMESTAMP"))
        conn.commit()
    except: 
        conn.rollback()
        
    try: 
        conn.execute(text("ALTER TABLE users ADD COLUMN last_eval_date TIMESTAMP"))
        conn.commit()
    except: 
        conn.rollback()
        
    try: 
        conn.execute(text("ALTER TABLE users ADD COLUMN eval_count INTEGER DEFAULT 0"))
        conn.commit()
    except: 
        conn.rollback()
        
    try: 
        conn.execute(text("ALTER TABLE users ADD COLUMN notes VARCHAR DEFAULT ''"))
        conn.commit()
    except: 
        conn.rollback()
    ...
    try: 
        conn.execute(text("ALTER TABLE schedules ADD COLUMN is_override INTEGER DEFAULT 0"))
        conn.commit()
    except: 
        conn.rollback()
        
    try: 
        conn.execute(text("ALTER TABLE user_groups ADD COLUMN is_flexible INTEGER DEFAULT 0"))
        conn.commit()
    except: 
        conn.rollback()
        
    try: 
        conn.execute(text("ALTER TABLE activities ADD COLUMN color VARCHAR DEFAULT '#0A84FF'"))
        conn.commit()
    except: 
        conn.rollback()

# Seedowanie domyślnych danych
with SessionLocal() as db:
    users_to_fix = db.query(User).filter(User.hire_date == None).all()
    for u in users_to_fix:
        # Zabezpieczenie starych kont datą z przeszłości, żeby uniknąć crashy
...
# ==========================================
# 3. SYMULATOR WMS (STYCZEŃ - MAJ 2026)
# ==========================================
...
def run_simulation(db: Session = Depends(get_db)):
    ...
    # Czyścimy stare wpisy z tego okresu dla czystości symulacji
    ...
    db.query(Competence).delete() # Reset ocen do czysta
    
    for u in users:
        # Ustawienie "stażu" pracownika na przed styczeń
        u.hire_date = datetime(2025, 12, 1) 
        u.eval_count = 2 # Przeszedł oceny wstępne (14 dni i 1.5 mc)
        u.last_eval_date = datetime(2026, 2, 15) # Odpali to alert "3 miesiące" w połowie Maja
        
        # Kompetencje (Wszystkie na 3)
        ...
        while curr_d <= end_date:
            ...
            if curr_d.weekday() >= 5:
                ...
                if rand_val <= 5: 
                    db.add(Schedule(username=u.name, date_str=d_str, activity="Chory 🤒", hours="", is_override=1))
                elif rand_val <= 10: 
                    db.add(Schedule(username=u.name, date_str=d_str, activity="Urlop 🌴", hours="", is_override=1))
                elif rand_val <= 12: 
                    db.add(Schedule(username=u.name, date_str=d_str, activity="No Show ❌", hours="", is_override=1))
                else:
                    if acts:
    ...
    db.commit()
    return {"ok": True, "msg": "Zakończono generowanie historii pracy (Sty - Maj 2026). Pracownicy otrzymali alerty HR o Ocenie Okresowej (3 mc)."}

# ==========================================
# 4. ENDPOINTY PUBLICZNE I AUTORYZACJA
# ==========================================
@app.get("/")
def serve_vmax():
...
def login(req: dict, db: Session = Depends(get_db)):
    ...
    if r == "ADMIN":
        ...
        # Uprawnienia dla warstwy managerskiej (wszyscy ci mają dostęp do Panelu Zarządzania)
...
def change_pin(req: dict, db: Session = Depends(get_db)):
    ...
    if name == root_login:
        if not is_admin and oldP != root_pass: 
            return {"ok": False, "msg": "Stare hasło jest błędne"}
        p_setting = db.query(GlobalSetting).filter(GlobalSetting.setting_type == "admin_pass").first()
        if p_setting: 
            p_setting.value = newP
        db.commit()
        return {"ok": True, "msg": "Hasło Głównego Admina zostało zmienione!"}

    user = db.query(User).filter(User.name == name).first()
    if not user: 
        return {"ok": False, "msg": "Nie znaleziono użytkownika"}
    if not is_admin and user.pin != oldP: 
        return {"ok": False, "msg": "Stary PIN jest błędny"}
...
def get_emp_dash(req: dict, db: Session = Depends(get_db)):
    ...
    if not shifts: 
        shifts = ["07:00-15:00", "08:00-16:00"]
...
def submit_request_batch(req: dict, db: Session = Depends(get_db)):
    ...
    if name == root_login: 
        ...
        user_db = db.query(User).filter(User.name == name).first()
        if not user_db: 
            return {"ok": False, "msg": "Błąd użytkownika"}
...
def get_admin_data(db: Session = Depends(get_db)):
    ...
    for u in db_users:
        ...
        if u.eval_count == 0 and days_since_hire >= 14:
            hr_alerts.append({"id": f"hr_{u.name}_0", "name": u.name, "date": now.strftime("%Y-%m-%d"), "type": "Ocena (14 dni)", "hrs": "", "ts": now.strftime("%Y-%m-%d %H:%M")})
        elif u.eval_count == 1 and days_since_hire >= 45:
            hr_alerts.append({"id": f"hr_{u.name}_1", "name": u.name, "date": now.strftime("%Y-%m-%d"), "type": "Ocena (1.5 mc)", "hrs": "", "ts": now.strftime("%Y-%m-%d %H:%M")})
    ...
    return {
        ...
        "plannerActivities": planner_activities,
        "activityColors": activityColors, 
        "shifts": shifts, 
...
def get_hr_details(req: dict, db: Session = Depends(get_db)):
    ...
    # Filtrowanie zakresu dat absencji
    now = get_now()
    ...
    schedules = db.query(Schedule).filter(Schedule.username == username).all()
    stats = {"W pracy": 0, "Chory 🤒": 0, "Urlop 🌴": 0, "Wolne 🏠": 0, "No Show ❌": 0}
        
    for s in schedules:
        try:
            d = datetime.strptime(s.date_str, "%Y-%m-%d")
            if d_from.date() <= d.date() <= d_to.date():
                if s.activity in tracked_acts:
                    stats[s.activity] += 1
                elif s.activity and s.activity not in ["Brak planu", "Wyczyść"]:
                    stats["W pracy"] += 1
        except: 
            pass
        
    eval_logs = db.query(EvaluationLog).filter(EvaluationLog.username == username).order_by(desc(EvaluationLog.eval_date)).all()
    history = [{"date": l.eval_date.strftime("%Y-%m-%d"), "rating": l.rating, "notes": l.notes_snapshot} for l in eval_logs]
...
def save_hr_details(req: dict, db: Session = Depends(get_db)):
    ...
    rating_val = int(req.get("rating", 3)) 
    ...
    if user_db:
        ...
        if is_eval:
            ...
            db.add(EvaluationLog(username=username, rating=rating_val, notes_snapshot=notes))
        db.commit()
        
    return {"ok": True, "msg": "Zapisano ewaluację i wyczyszczono alert!" if is_eval else "Zapisano notatki HR."}

# ==========================================
# 8. ZARZĄDZANIE GRAFIKIEM (PLANNER)
# ==========================================
...
def resolve_mass_alerts(req: dict, db: Session = Depends(get_db)):
    ...
    # Blokada na zamykanie alertów HR przez masowe kliknięcie
    ...
    if hr_skipped > 0: 
        msg += f" (Pominięto {hr_skipped} alertów HR)"
...
def copy_daily(req: dict, db: Session = Depends(get_db)):
    src_date = req.get("sourceDate")
    t_start = req.get("targetStart")
    t_end = req.get("targetEnd")
...
def download_db(db: Session = Depends(get_db)):
    ...
    raise HTTPException(status_code=404, detail="Baza nie istnieje (Używasz PostgreSQL?)")

# ==========================================
# 10. CMS I USTAWIENIA GŁÓWNE
# ==========================================
...
def cms_settings(req: dict, db: Session = Depends(get_db)):
    ...
    try:
        if action == "ADD":
            if t == "employee": 
                if db.query(User).filter(User.name == val).first(): 
                    return {"ok": False, "msg": "Użytkownik już istnieje!"}
                new_id = generate_global_id(db)
                db.add(User(
                    global_id=new_id, 
                    name=val, 
                    pin="1111", 
                    role=req.get("role", "EMPLOYEE"), 
                    group_name=req.get("group", "Magazyn osoby stałe")
                ))
            elif t == "shift": 
                db.add(GlobalSetting(setting_type="shift", value=val))
            elif t == "activity": 
                act = db.query(Activity).filter(Activity.name == val).first()
                if act: 
                    act.color = req.get("color", "#0A84FF")
                else: 
                    db.add(Activity(name=val, color=req.get("color", "#0A84FF")))
            elif t == "group": 
                if not db.query(UserGroup).filter(UserGroup.name == val).first():
                    db.add(UserGroup(
                        name=val, 
                        emp_type=req.get("emp_type", "Stały"), 
                        is_flexible=req.get("is_flexible", 0), 
                        allowed_activities="[]"
                    ))
            ...
            if grp: 
                grp.is_flexible = req.get("is_flexible", 0)
            ...
            if t == "employee": 
                ...
                db.query(GlobalSetting).filter(GlobalSetting.setting_type == "shift", GlobalSetting.value == val).delete()
            elif t == "activity": 
